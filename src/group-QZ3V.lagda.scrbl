@title{關於交換群 Eckmann–Hilton argument}
@taxon{Proposition}
...
@p{因為等一下就會用到，首先證明如果 @code{G} 是群，那 @code{G × G} 是群}
...
@h2{Eckmann–Hilton argument}

@p{如果群的二元運算 @code{a ∙ b} 本身就是一個 group homomorphism，那麼 @code{G} 是交換群}
...
@p{反過來也很明顯，我們只需要證明交換會符合 interchange law}
