...
@p{參考 @external["https://ncatlab.org/nlab/show/heap"]{nLab heap}}

@p{Heap presentation 用一個三元運算 @code{p}（標準模型讀作 @code{p x y z = (x ∙ y ⁻¹) ∙ z}），
...
@p{要把一個 Heap 變回 Group，必須額外指定一個 basepoint @code{e₀ : A}}
