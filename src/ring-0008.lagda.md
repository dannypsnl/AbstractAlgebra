...
proposition-4 {_} {R} u = I , II
  where
  I : is-left-unit u → has-section (left-action u)
  I (v , mul-to-one) = left-action v , iso-to-id
    where
  ...
  II : has-section (left-action u) → is-left-unit u
  II (sec , to-id) = sec 1r , to-id 1r
```
